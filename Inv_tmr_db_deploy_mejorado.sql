...
-- =============================================================================
-- ADVERTENCIA DE SEGURIDAD (TASK-001):
--   Este script contiene DROP DATABASE IF EXISTS en la sección de creación.
--   NUNCA re-ejecutar sobre un ambiente de producción con datos existentes.
--   Solo para primer despliegue en ambiente limpio o contenedor sandbox.
-- =============================================================================
...
CREATE DATABASE "Inv_tmr_db"
    ...
    LC_CTYPE    = 'en_US.utf8'
    TEMPLATE    = template0;
COMMENT ON DATABASE "Inv_tmr_db"
    IS 'Base de datos unificada: Inventario de Activos (isc-inv) + Time Report RRHH (isc-tmr). Estándar bancario Ecuador.';
...
COMMENT ON SCHEMA administracion
    IS 'Personal, cargos, clientes, modos de trabajo y datos maestros de RRHH';

CREATE SCHEMA IF NOT EXISTS inventario;
COMMENT ON SCHEMA inventario
    IS 'Equipos, activos tecnológicos, proveedores y facturas de adquisición';

CREATE SCHEMA IF NOT EXISTS autenticacion;
COMMENT ON SCHEMA autenticacion
    IS 'Usuarios, roles, permisos, sesiones y aplicaciones del sistema';

CREATE SCHEMA IF NOT EXISTS auditoria;
COMMENT ON SCHEMA auditoria
    IS 'Registro histórico de cambios en entidades críticas del sistema';

CREATE SCHEMA IF NOT EXISTS time_report;
COMMENT ON SCHEMA time_report
    IS 'Gestión de proyectos, actividades diarias, permisos y reporte de horas';

-- ---------------------------------------------------------------------------
-- 1. tbl_administracion_catalogo
-- Origen: Tabla nueva. 
-- Catálogo general para clasificaciones varias (tipo, estado, prioridad, etc.)
-- AUT: Autenticacion, ADM: Administración, AUD: Auditoría, TMR: Time Report, INV: Inventario
-- ---------------------------------------------------------------------------
CREATE TABLE administracion.tbl_administracion_catalogo (
    Id                  INT GENERATED ALWAYS AS IDENTITY NOT NULL,
    TipoCatalogo        VARCHAR(3)     NOT NULL CHECK (TipoCatalogo IN ('AUT','ADM','AUD','TMR','INV')),
...
CREATE TABLE administracion.tbl_administracion_catalogo_detalle (
    ...
    CodigoValor         VARCHAR(4)     NOT NULL,
...
CREATE TABLE administracion.tbl_administracion_persona (
    ...
    CONSTRAINT fk_administracion_persona_tipo_identificacion
        ...
        REFERENCES administracion.tbl_administracion_catalogo_detalle(Id),
    CONSTRAINT uq_administracion_persona_identificacion UNIQUE (NumeroIdentificacion)
);
...
CREATE TABLE administracion.tbl_administracion_empleado (
    ...
    CONSTRAINT fk_administracion_empleado_departamento
        FOREIGN KEY (IdDepartamento)
        REFERENCES administracion.tbl_administracion_catalogo_detalle(Id),
...
CREATE TABLE autenticacion.tbl_autenticacion_usuario (
    ...
    CONSTRAINT fk_autenticacion_usuario_empleado
        FOREIGN KEY (IdEmpleado)
        REFERENCES administracion.tbl_administracion_empleado(Id),
    CONSTRAINT uq_autenticacion_usuario_nombre UNIQUE (NombreUsuario)
);
...
-- ---------------------------------------------------------------------------
-- 32. tbl_inventario_factura_detalle
-- Origen: 
-- --------------------------------------------------------------------------
CREATE TABLE inventario.tbl_inventario_factura_detalle (
    Id                  INT GENERATED ALWAYS AS IDENTITY NOT NULL,
    ...
    IpCreacion          VARCHAR(45)     NOT NULL,
    IpModificacion      VARCHAR(45),
    CONSTRAINT pk_inventario_factura_detalle PRIMARY KEY (Id),
    CONSTRAINT fk_inventario_factura_detalle_factura
...
-- ---------------------------------------------------------------------------
-- 35. tbl_inventario_caracteristica_equipo
-- Origen: 
-- --------------------------------------------------------------------------
...
CREATE TABLE time_report.tbl_time_report_actividad_diaria (
    ...
    CantidadHoras           NUMERIC(5,2)    NOT NULL CHECK (CantidadHoras > 0 AND CantidadHoras <= 24),
...
CREATE TABLE time_report.tbl_time_report_permiso (
    ...
    CONSTRAINT fk_time_report_permiso_aprobador
        ...
        REFERENCES administracion.tbl_administracion_empleado(Id),
    CONSTRAINT chk_time_report_permiso_fechas CHECK (FechaFin >= FechaInicio)
);
...
CREATE INDEX idx_inv_factura_detalle         ON inventario.tbl_inventario_factura_detalle(IdFactura);
...
BEGIN
    ...
    adq AS (
        ...
        JOIN inventario.tbl_inventario_factura_detalle df ON df.IdFactura = f.Id
...
BEGIN
    ...
    WITH ce AS (
        SELECT COUNT(*) FILTER (WHERE Activo = TRUE)  AS total_activo,
               ...
               COUNT(*) FILTER (
                   WHERE IdEstado = (SELECT Id FROM administracion.tbl_administracion_catalogo_detalle 
...
BEGIN
    ...
    SELECT e.Id, e.Marca, e.Modelo, e.NumeroSerie,
           ...
           ua.FechaAsignacion,
           (p.PrimerNombre||' '||p.ApellidoPaterno)::TEXT
...
BEGIN
    ...
    SELECT COALESCE(c.NombreComercial,c.RazonSocial)::VARCHAR, p.Nombre::VARCHAR,
           ep.Valor::VARCHAR,
           (pers.PrimerNombre||' '||pers.ApellidoPaterno)::TEXT,
    ...
    GROUP BY COALESCE(c.NombreComercial,c.RazonSocial), p.Nombre,
             ep.Valor, pers.PrimerNombre, pers.ApellidoPaterno, p.Presupuesto
    ORDER BY 1,2;
END; $$;
...
BEGIN
    ...
    resumen AS (
        SELECT e.Id,
               (p.PrimerNombre||' '||p.ApellidoPaterno)::TEXT AS nombre,
        ...
        WHERE e.Activo=TRUE
        GROUP BY e.Id, p.PrimerNombre, p.ApellidoPaterno, p.NumeroIdentificacion
    )
...
BEGIN
    ...
    WHERE p.Activo=TRUE ORDER BY pers.PrimerNombre, pers.ApellidoPaterno, p.Nombre;
END; $$;
...
BEGIN
    ...
    SELECT DISTINCT e.Id,
           (pers.PrimerNombre||' '||pers.ApellidoPaterno),
...
BEGIN
    ...
    RAISE NOTICE '=========================================================';
END $$;
-- TASK-002: Cierre del wrapper transaccional — commit atómico de todo el despliegue
COMMIT;
